...
def count_continuing_ps_hits(
    ...
    """Count PS detections connected to detections in recent timesteps.

    A current vertex is counted when any mesh vertex within ``radius`` was
    present in one of the previous ``history_steps`` timesteps.
    """
    ...
    neighbourhood_cache: dict[int, set[int]] = {}
    ...
    for time_value, current_vertices in timesteps:
        ...
        for vertex in current_vertices:
            ...
            if vertex not in neighbourhood_cache:
                neighbourhood_cache[vertex] = set(
                    int(item)
                    for item in tree.query_ball_point(
                        mesh_coordinates[vertex],
                        radius,
                    )
                )

            if neighbourhood_cache[vertex].intersection(previous_vertices):
                counts[vertex] += 1
        ...
        history.append(set(int(vertex) for vertex in current_vertices))
...
def analyze_ps_coordinates(
    ...
    *,
    ...
    """Run the complete coordinate-based PS analysis."""
    mesh_coordinates = _prepare_mesh_coordinates(mesh_coordinates)
