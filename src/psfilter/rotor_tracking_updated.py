...
Mesh distances are Euclidean. Surface- or geodesic-aware matching is left for
future work.
"""
...
def _global_track_point_assignment(
    ...
    """Return globally optimal one-to-one track/point assignments.

    The cost construction first maximizes the number of valid matches and,
    among assignments with the same cardinality, minimizes total Euclidean
    distance. Each row has private dummy columns that represent leaving a
    track unmatched at the current timestep.
    """
    ...
    point_coordinates = mesh_coordinates[
        np.asarray(current_vertices, dtype=int)
    ]
    track_coordinates = np.asarray(
        [track.last_coordinate for track in active_tracks],
        dtype=float,
    )

    distances = np.linalg.norm(
        track_coordinates[:, None, :] - point_coordinates[None, :, :],
        axis=2,
    )

    # One lost valid match must cost more than any possible change in the
    # total distance of all remaining matches.
    ...
    # Tiny deterministic perturbations only resolve exact ties.
...
def build_rotor_tracks(
    ...
    """Build tracks using global one-to-one assignment at each timestep."""
    ...
    if not np.isfinite(match_radius) or match_radius <= 0:
        raise ValueError(
            "match_radius must be positive and finite."
        )

    if max_gap_steps < 1:
        raise ValueError(
            "max_gap_steps must be at least 1."
        )
    ...
    for step_index, (time_value, vertices_array) in enumerate(timesteps):
        # Retire tracks that are already too old to match this timestep.
...
def _split_indices_by_rolling_drift(
    ...
    *,
    ...
    """Split one temporal segment at rolling-window drift violations.

    For each current point, the anchor is the latest point at or before
    ``current_time - drift_window_time`` within the current piece. When the
    current-to-anchor displacement exceeds the allowed distance, the current
    point starts a new piece. The scan then restarts from that point.
    """
    if indices.size == 0:
        return [], []
    ...
    while piece_start_position < indices.size:
        ...
        for local_position in range(
            ...
            displacement = float(
                np.linalg.norm(
                    coordinates[current_global_index]
                    - coordinates[anchor_global_index]
                )
            )

            if displacement > maximum_rolling_drift_distance:
        ...
        pieces.append(
            indices[piece_start_position:split_position].copy()
        )
...
def split_track_into_final_segments(
    ...
    *,
    ...
    """Apply temporal-gap splitting followed by rolling-drift splitting."""
...
def _maximum_rolling_displacement(
    ...
    """Return the largest displacement across the rolling time window."""
    times = segment.times
    ...
    for current_index in range(1, times.size):
        ...
        distance = float(
            np.linalg.norm(
                coordinates[current_index] - coordinates[anchor_index]
            )
        )
